...
environment.def("echo", function (...args) {
  args = args.map((item) => (typeof item === "object" && item != null ? Object.values(item).join(" ") : item))
...
`
  ...
  assert(`${_fruits} Copy=("\${Fruits[@]}")`, ["Apple", "Banana", "Orange", "Cherry"])
  assert(`${_fruits} Veggies = ('Tomato' 'Cucumber' 'Carrot'); Double=("\${Fruits[@]}" "\${Veggies[@]}")`, [
    "Apple",
    "Banana",
    "Orange",
    "Cherry",
  ...
  assert(`Fruits = (\`get_fruits\`); Veggies = (\`get_fruits "veggies"\`); Double=("\${Fruits[@]}" "\${Veggies[@]}")`, [
    "Apple",
    "Banana",
    "Orange",
    "Cherry",
...
sounds[wolf]="howl"
`
  assert(`${_sounds}`, { dog: "bark", cow: "moo", bird: "tweet", wolf: "howl" })
  assert(`${_sounds} test = "\${sounds[cow]}"`, "moo")
  assert(`${_sounds} str = "\${sounds[@]}"`, ["bark", "moo", "tweet", "howl"])
  assert(`${_sounds} str = "\${!sounds[@]}"`, ["dog", "cow", "bird", "wolf"])
  assert(`${_sounds} str = "\${#sounds[@]}"`, 4)
  assert(`${_sounds} unset sounds[cow]; str = "\${sounds[@]}"`, ["bark", "tweet", "howl"])
  assert(
    `
    ${_sounds}
    values = ""
...
console.timeLog("✅ Dictionaries")

console.time("✅ Numeric Calculation")
assert(`$((2+2*2))`, 6)
assert(`$(((2+2)*2))`, 8)
for (let i = 0; i <= 100; i++) assert(`$(($RANDOM%20))`, (res) => res >= 0 && res <= 20)
console.timeLog("✅ Numeric Calculation")

environment.def("[[", function (type = "fruits") {
