...
const parseValue = function (str, values, callback) {
  ...
  while (text.length) {
    ...
    if (text.match(new RegExp(`^${PLACEHOLDER}`))) {
      ...
      if (!value || Array.isArray(value) || [null, undefined, ""].includes(value)) {
        ...
        callback({ tagName: "i", props: { style: `padding-right: ${value.length}ch` } })
        text = text.replace(new RegExp(PLACEHOLDER), "")
      } else {
        callback(value)
        text = text.replace(new RegExp(PLACEHOLDER), "")
      ...
      const _regexp = new RegExp(`^[^${PLACEHOLDER}]+`)
      const _matched = text.match(_regexp)
      if (_matched) {
        const [string] = _matched
        callback(string)
...
export { ast, html, PLACEHOLDER, parseString }
